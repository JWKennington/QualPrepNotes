...
\eqn{\vec{v}_f = \vec{V} + \vec{v}_r + \vec{\omega}\times\vec{r}}
...
The inertia tensor $\tens{I} \in S^2\wrap{\RThree} \subset \mathcal{T}^{2}_{0}$ such that $\vec{I}: \RThree \times \RThree \rightarrow \R$. Where when acting on angular velocity $\omega$, produces $\tens{I}\wrap{\omega, \omega} = T_{\text{rot}}$. We can raise an index on $\tens{I}$ to produce a linear operator $\tens{I}: \RThree \to \RThree$ such that $\vec{L} = \tens{I}\vec{\omega}$
%   Basic Definition
%   Tensor Definition
