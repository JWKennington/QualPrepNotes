...
\eqn[eq:cm-rigid-motion]{\boxed{\wrap{I_i - I_j}\omega_i\omega_j - \sum_k\wrap{I_k \dot{\omega}_k - N_k}\varepsilon_{ijk} = 0}}
% Stability
...
\eqn[eq:cm-rigid-sym-top]{
\begin{split}
	\omega_1 &= A\cos{\Omega t}\\ 
	\omega_2 &= A\sin{\Omega t}
\end{split}
\quad\quad \mathrm{where}\ \Omega = \frac{I_3 - I_1}{I_1}\omega_3 
}
% Point fixed
...
\eqn[eq:cm-rigid-top-lag]{\Lag = \half I_1 \wrap{\dot{\phi}^2\sin^2\theta + \dot{\theta}^2} + \half I_3 \wrap{\dot{\phi}\cos\theta + \dot{\psi}}^2 - Mgh\cos\theta}
\eqn{E' = \half I_1 \dot{\theta}^2 + V(\theta)\where V(\theta)=\frac{\wrap{p_{\phi} - p_{\psi}\cos\theta}^2}{2I_1 \sin^2\theta} + Mgh\cos\theta}
...
% Intro - Coupling / Normal coords / Normal modes
...
\eqn[eq:cm-coupled-conv]{\begin{split}
	M\ddot{x}_1 + \wrap{\kappa + \kappa_{12}}x_1 - \kappa_{12}x_2 &= 0\\
	M\ddot{x}_2 + \wrap{\kappa + \kappa_{12}x_2} - \kappa_{12}x_1 &= 0
\end{split}\quad\quad
\begin{split}
...
\end{split}}
\eqn{\omega=\sqrt{\frac{\kappa +\kappa_{12} \pm \kappa_{12}}{M}}\quad\quad
	 \omega_1 = \sqrt{\frac{\kappa +2\kappa_{12}}{M}}\quad\quad
	 \omega_2=\sqrt{\frac{\kappa}{M}}\quad\quad
	 \omega_0=\sqrt{\frac{\kappa + \kappa_{12}}{M}}}
\texfig[0.95]{cm-coupled-setup}{A system of two coupled oscillators at equilibrium and oscillating at the two normal modes $\omega_1$ and $\omega_2$.}\\
% Normal coordinates for this system
...
\begin{split}
	M\ddot{\eta}_1 + \wrap{\kappa + 2\kappa_{12}}\eta_1 &= 0\\
	M\ddot{\eta}_2 + \kappa \eta_2 &= 0
\end{split}\quad\quad\quad
\begin{split}
...
\end{split}}
% weak coupling
In the weak coupling limit, $\kappa_{12} << \kappa$, the motion may be further simplified in terms of a coupling coefficient $\varepsilon$. The weak coupling motion resembles a higher frequency wave modulated in a lower frequency envelope, since $D\cos\varepsilon\omega_0 t$ and $D\sin\varepsilon\omega_0 t$ vary slowly with time for small $\varepsilon$.
\eqn{\begin{split}
	x_1(t) &= \wrap{D\cos{\varepsilon\omega_0 t}}\cos{\omega_0 t} \\
	x_2(t) &= \wrap{D\sin{\varepsilon\omega_0 t}}\sin{\omega_0 t}
\end{split}\quad\quad
\varepsilon \equiv \frac{\kappa_{12}}{2\kappa} << 1\quad\quad
\begin{split}
	\omega_1 &\approx \omega_0\wrap{1 + \varepsilon} \\
	\omega_2 &\approx \omega_0\wrap{1 - \varepsilon}
\end{split}}

% General-coupled oscillators - Eigenmodes/degeneracy
\subsection{General Coupled Oscillators}
...
\eqn{T=\half\sum_{j,k}m_{jk}\dot{q}_j\dot{q}_k\quad\quad U=\half\sum_{j,k}A_{jk}q_j q_k\quad\quad A_{jk}=A_{kj}=\frac{\p^2 U}{\p q_j \p q_k}\bigg\rvert_0}
...
\eqn{\sum_j\wrap{A_{jk}q_j + m_{jk}\ddot{q}_j}=0\quad\quad \sum_j\wrap{A_{jk}-\omega^2 m_{jk}}a_j=0\quad\quad \norm{A_{jk} - \omega^2 m_{ij}}=0}
...
\eqn{q_j(t)=\sum_r a_{jr} e^{i\wrap{\omega_r t - \delta_r}}=\sum_r a_{jr}\cos\wrap{\omega_r t - \delta_r}}
...
\eqn{U=\half\wrap{\kappa + \kappa_{12}}\wrap{x_1^2 + x_2^2} - \kappa_{12}x_1 x_2\quad\quad T = \half M\wrap{\dot{x}_1^2 + \dot{x}_2^2}}
\eqn{A_{11}=\frac{\p^2 U}{\p x_1^2}\bigg\rvert_0=\kappa + \kappa_{12}\quad\quad
A_{12}=A_{21}=\frac{\p^2 U}{\p x_1\p x_2}\bigg\rvert_0=-\kappa_{12}\quad\quad
A_{22}=\frac{\p^2 U}{\p x_2^2}\bigg\rvert_0=\kappa + \kappa_{12}
}
\eqn{m_{11}=m_{22}=M\quad\quad m_{12}=m_{21}=0}
\eqn{\begin{vmatrix}
\kappa + \kappa_{12} - M\omega^2 & -\kappa_{12} \\
-\kappa_{12} & \kappa + \kappa_{12} - M\omega^2
\end{vmatrix}=0 \implies \omega=\sqrt{\frac{\kappa + \kappa_{12} \pm \kappa_{12}}{M}}}
% Normal Coordinates
...
\eqn{q_j(t)=\sum_r \beta_r e^{i\omega_r t}\quad\quad\quad \beta_r = \alpha_r e^{i\delta_r}}
...
\eqn{\boxed[2\fboxsep]{\eta_r(t)\equiv \beta_r e^{i\omega_r t}\quad\quad q_j(t)=\sum_r a_{jr}\eta_r(t)}}

% Molecular Vibrations - READ ONLY / SKIP
% Degeneracy - example 3 linearly coupled plane pendula
...
\texfig[0.6]{cm-coupled-degen}{A system of three pendula suspended from a slightly yielding support.}
...
\eqn{q_j(t)=\sum_r \wrap{j\frac{r\pi}{n+1}}\wrap{\mu_r \cos \omega_r t - \nu_r\sin \omega_r t}\where[1] \omega_r= 2\sqrt{\frac{\tau}{md}} \sin \wrap{\frac{r\pi}{2\wrap{n+1}}}}
\eqn{\mu_r=\frac{2}{n+1}\sum_j q_j(0)\sin\wrap{j\frac{r\pi}{n+1}}\quad\quad 
\nu_r=-\frac{2}{\omega_r\wrap{n+1}}\sum_j\dot{q}(0)\sin\wrap{j\frac{r\pi}{n+1}}}
\texfig[0.6]{cm-coupled-loaded}{Schematic diagram of loaded string.}
...
\eqn{q(x, t)=\sum_r \eta_r(t) \sin \wrap{\frac{r\pi x}{L}}\where[1] j\frac{r\pi}{\wrap{n+1}}=r\pi\frac{jd}{\wrap{n+1}d}=r\pi\frac{x}{L}}
...
\eqn{q(x,0)=\sum_r \mu_r \sin\wrap{\frac{r\pi x}{L}}\quad\quad \dot{q}(x,0)=-\sum_r \omega_r\nu_r \sin\wrap{\frac{r\pi x}{L}} \quad\quad \omega_r=\frac{r\pi}{L}\sqrt{\frac{\tau}{\rho}}}
\eqn{\mu_r=\frac{2}{L}\int_0^L q(x, 0)\sin\wrap{\frac{r\pi x}{L}}dx\quad\quad \mu_r=\frac{2}{L}\int_0^L q(x, 0)\sin\wrap{\frac{r\pi x}{L}}dx}
...
\eqn{T = \half\rho\int_0^L \wrap{\pd{q}{t}}^2 dx=\frac{\rho L}{4}\sum_r \dot{\eta}_r^2=\frac{\rho L}{4}\sum_r \omega_r^2 \wrap{\mu_r\sin\omega_r t + \nu_r\cos \omega_r t}^2}
\eqn{U = \half\tau\int_0^L \wrap{\pd{q}{x}}^2 dx=\frac{\rho L}{4}\sum_r \omega_r^2 \eta_r^2=\frac{\rho L}{4}\sum_r \omega_r^2 \wrap{\mu_r\cos\omega_r t - \nu_r\sin \omega_r t}^2}
\eqn{E = T + U = \frac{\rho L}{4}\sum_r \omega_r^2 \wrap{\mu_r^2 + \nu_r^2}=\frac{\rho L}{4}\sum_r \omega_r \norm{\beta_r}^2}
From the above it is possible to derive the one-dimensional wave equation (and vice-versa). Recall the discrete equation of motion, $\frac{m}{d}\ddot{q}=\frac{\tau}{d}\wrap{q_{j-1} - q_j}-\frac{\tau}{d}\wrap{q_{j+1} - q_j}$, and take limit as $d \to 0$:
\eqn{\frac{q_j - q_{j+1}}{d}\to\frac{q(x) - q(x+d)}{d}\to -\pd{q}{x}\bigg\rvert_{x+d/2}\quad\quad \text{similarly}\quad\quad \frac{q_{j-1} - q_j}{d}\to-\pd{q}{x}\bigg\rvert_{x-d/2}}
\eqn{\lim_{d\to0}\tau\wrap{\frac{\pd{q}{x}\rvert_{x+d/2} - \pd{q}{x}\rvert_{x-d/2}}{d}}=\tau\pd[2]{q}{x}\bigg\rvert_{x}=\tau\pd[2]{q}{x}}
\eqn{\boxed{\pd[2]{q}{x}=\frac{\rho}{\tau}\pd[2]{q}{t}}}

% Wave Equation
\subsection{Wave Equation}
The general wave equation for a \textit{wave function} $\Psi$ is stated in Eq. \ref{eq:cm-cont-wave}. Using a transform of variables $\xi\equiv x + vt$, $\eta\equiv x - vt$ the wave function can be shown to be a superposition of forward and backward propagating waves $\Psi = f(\xi) + g(\eta)$. 
\eqn[eq:cm-cont-wave]{\pd[2]{\Psi}{x} - \frac{1}{\nu^2}\pd[2]{\Psi}{t}\quad\quad
\Psi(x,t)=\psi(x)e^{i\omega t}}
Using a separation of variables such that $\Psi(x,t)=\psi(x)\cdot\chi(t)$, the resulting separate ODE equations become $\psi_{xx} + \wrap{\omega^2/\nu^2}\psi=0$ and $\chi_{tt}+\omega^2\chi=0$, which possess the familiar solutions given below
\eqn{\psi(x)=Ae^{i\wrap{\omega/\nu}x} + Be^{i\wrap{\omega/\nu}x}\quad\quad 
\chi(t)=Ce^{i\omega t} + D e^{-i\omega t}}
The one-dimensional version of this equation is referred to as the \textit{Helmholtz} equation with a coefficient called the \textit{wavenumber} $k$:
\eqn{\frac{d^2\psi}{dx^2}+k^2\psi=0 \quad\quad
	k\equiv\frac{\omega}{\nu}=\frac{2\pi}{\lambda}}

% Phase Velocity
\subsection{Phase Velocity}
Considering solutions of the form $\Psi(x,t)=Ae^{i\wrap{\omega t - kx}}$, which represent a single-frequency waveform traveling to the right, the \textit{phase velocity} $V$ corresponds to the velocity with which the wave form propagates. The \textit{phase} of the wave $\phi=\omega t - kx$. The velocity can be computed assuming $d\phi=0$.
\eqn{V=\frac{dx}{dt}=\frac{\omega}{k}}
When $V=V(k)$, the corresponding function $\omega(k)$ is called the \textit{dispersion relation}. For example, a vibrating string exhibits dispersion:
\eqn{V=\frac{\omega}{k}=\sqrt{\frac{\tau d}{m}}\frac{\norm{\sin\wrap{kd/2}}}{kd/2}\quad\quad
V\wrap{\lambda \to \infty}=\sqrt{\frac{\tau d}{m}} = \sqrt{\frac{\tau}{\rho}}}

% Group Velocity
\subsection{Group Velocity}
When multiple single-frequency waves are propagating together, the combined waveform travels with a speed called the \textit{group velocity} $U$, which can be computed by:
\eqn{U=\frac{dx}{dt}\approx\frac{d\omega}{dk}}
The group velocity is only applied to cases that can be represented as a \textit{wave packet}, Eq. \ref{eq:cm-cont-packet}.
\eqn[eq:cm-cont-packet]{\Psi(x,t)=\int_{k_0-\Delta k}^{k_0+\Delta k}A(k)e^{i\wrap{\omega t - kx}}dk}
It's worth noting that since information is transmitted by forming a wave packet, the group velocity, not the phase velocity, corresponds to the speed at which a signal is transmitted.
